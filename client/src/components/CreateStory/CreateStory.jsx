...
function CreateStory() {
    ...
    const [chapters, setChapters] = useState([]);
    const [currentChapter, setCurrentChapter] = useState({ title: '', content: '', choices: [{ choiceText: '', nextChapterIndex: null, isEnd: false }], endingContent: '' });
    const [chapterIndexToEdit, setChapterIndexToEdit] = useState(null);
    const [currentChoiceIndex, setCurrentChoiceIndex] = useState(null);
    ...
    const addChoice = () => {
        if (currentChapter.choices.length < 3) {
            setCurrentChapter(prevState => ({
                ...prevState,
                choices: [...prevState.choices, { choiceText: '', nextChapterIndex: null, isEnd: false }]
            }));
        } else {
            alert("You can only add up to three choices.");
        }
    ...
    const handleChapterSubmit = (event) => {
        ...
        const newChapter = {
            ...
            content: currentChapter.content,
            choices: currentChapter.choices,
            endingContent: currentChapter.endingContent
        };
        ...
        setCurrentChapter({ title: '', content: '', choices: [{ choiceText: '', nextChapterIndex: null, isEnd: false }], endingContent: '' });
        setIsChapterModalOpen(false);
        setChapterIndexToEdit(null);
    };
    ...
    const editChapter = (index) => {
        ...
        setCurrentChapter({
            ...
            content: chapterToEdit.content,
            choices: chapterToEdit.choices,
            endingContent: chapterToEdit.endingContent
        });
        setChapterIndexToEdit(index);
        setIsChapterModalOpen(true);
    };
                    ...
                    {currentChapter.choices.map((choice, index) => (
                        <div key={index} className="choice-wrapper">
                            <input
                                ...
                                required
                            />
                            <div className="choice-end-wrapper">
                                <input
                                    type="checkbox"
                                    checked={choice.isEnd}
                                    onChange={(e) => handleChoiceChange(index, 'isEnd', e.target.checked)}
                                />
                                <label>End Story</label>
                            </div>
                            <button type="button" onClick={() => removeChoice(index)}>Remove Choice</button>
                        </div>
                    ))}
